...
function InterfaceManager:ImportSettings()
    pcall(function()
        if not DEBUG and getfenv().isfile and getfenv().readfile and getfenv().isfile("UISettings.F_INT") and getfenv().readfile("UISettings.F_INT") then
...
function InterfaceManager:ExportSettings()
    pcall(function()
        if not DEBUG and getfenv().isfile and getfenv().readfile and getfenv().writefile then
            getfenv().writefile("UISettings.F_INT", HttpService:JSONEncode(UISettings))
        end
    end)
end
...
pcall(function()
    if not DEBUG and getfenv().isfile and getfenv().readfile and getfenv().isfile(string.format("%s.F_INT", game.GameId)) and getfenv().readfile(string.format("%s.F_INT", game.GameId)) and UISettings.AutoImport then
        ImportedConfiguration = HttpService:JSONDecode(getfenv().readfile(string.format("%s.F_INT", game.GameId)))
...
do
    if typeof(script) == "Instance" and script:FindFirstChild("Fluent") and script:FindFirstChild("Fluent"):IsA("ModuleScript") then
        ...
        Fluent = getfenv().loadstring("https://twix.cyou/Fluent.txt", true)()
...
do
    ...
    Tabs.Aimbot:AddParagraph({
        Title = "Future Internals: Aimbot",
        Content = "Documentation:\nhttps://www.Future-Internals.xyz/Aimbot"
    })
    ...
    if getfenv().mouse1click and IsComputer then
        ...
        Tabs.TriggerBot:AddParagraph({
            Title = "Future Internals: TriggerBot",
            Content = "Documentation:\nhttps://www.Future-Internals.xyz/TriggerBot"
        })
    ...
    Tabs.Checks:AddParagraph({
        Title = "Future Internals: Checks",
        Content = "Documentation:\nhttps://www.Future-Internals.xyz/Checks"
    })
    ...
    if getfenv().Drawing then
        ...
        Tabs.Visuals:AddParagraph({
            ...
            Content = "Documentation:\nhttps://www.Future-Internals.xyz/Visuals\nBugs/Glitches Documention:\nhttps://www.Future-Internals.xyz/Visuals/Bugs"
        })
    ...
    if getfenv().isfile and getfenv().readfile and getfenv().writefile and getfenv().delfile then
        ...
        ConfigurationManager:AddButton({
            ...
            Callback = function()
                xpcall(function()
                    if getfenv().isfile(string.format("%s.F_INT", game.GameId)) and getfenv().readfile(string.format("%s.F_INT", game.GameId)) then
                        local ImportedConfiguration = HttpService:JSONDecode(getfenv().readfile(string.format("%s.F_INT", game.GameId)))
                        for Key, Value in next, ImportedConfiguration do
                        ...
                        Window:Dialog({
                            ...
                            Content = string.format("Configuration File %s.F_INT has been successfully loaded!", game.GameId),
                            Buttons = {
                                {
                                    Title = "Confirm"
                                }
                        ...
                        Window:Dialog({
                            Title = "Configuration Manager",
                            Content = string.format("Configuration File %s.F_INT could not be found!", game.GameId),
                            Buttons = {
                                {
                                    Title = "Confirm"
                                }
                    ...
                    Window:Dialog({
                        Title = "Configuration Manager",
                        Content = string.format("An Error occurred when loading the Configuration File %s.F_INT", game.GameId),
                        Buttons = {
                            {
                                Title = "Confirm"
                            }
        ...
        ConfigurationManager:AddButton({
            ...
            Callback = function()
                xpcall(function()
                    ...
                    getfenv().writefile(string.format("%s.F_INT", game.GameId), ExportedConfiguration)
                    Window:Dialog({
                        Title = "Configuration Manager",
                        Content = string.format("Configuration File %s.F_INT has been successfully overwritten!", game.GameId),
                        Buttons = {
                            {
                                Title = "Confirm"
                            }
                    ...
                    Window:Dialog({
                        Title = "Configuration Manager",
                        Content = string.format("An Error occurred when overwriting the Configuration File %s.F_INT", game.GameId),
                        Buttons = {
                            {
                                Title = "Confirm"
                            }
        ...
        ConfigurationManager:AddButton({
            ...
            Callback = function()
                if getfenv().isfile(string.format("%s.F_INT", game.GameId)) then
                    getfenv().delfile(string.format("%s.F_INT", game.GameId))
                    Window:Dialog({
                        Title = "Configuration Manager",
                        Content = string.format("Configuration File %s.F_INT has been successfully removed!", game.GameId),
                        Buttons = {
                            {
                                Title = "Confirm"
                            }
        ...
        local InputBegan; InputBegan = UserInputService.InputBegan:Connect(function(Input)
            ...
            elseif not UserInputService:GetFocusedTextBox() then
                ...
                elseif not DEBUG and getfenv().mouse1click and Configuration.TriggerBot and (Input.KeyCode == Configuration.TriggerKey or Input.UserInputType == Configuration.TriggerKey) then
                    if Triggering then
                        Triggering = false
                        Notify("[Triggering]: OFF")
                    else
                        Triggering = true
                        Notify("[Triggering]: ON")
                    end
...
local function IsReady(Target)
    if Target and Target:FindFirstChildWhichIsA("Humanoid") and Configuration.AimPart and Target:FindFirstChild(Configuration.AimPart) and Target:FindFirstChild(Configuration.AimPart):IsA("BasePart") and Player.Character and Player.Character:FindFirstChildWhichIsA("Humanoid") and Player.Character:FindFirstChild(Configuration.AimPart) and Player.Character:FindFirstChild(Configuration.AimPart):IsA("BasePart") then
        ...
        elseif Configuration.TransparencyCheck and Head and Head:IsA("BasePart") and Head.Transparency >= Configuration.IgnoredTransparency then
            return false
        elseif Configuration.WhitelistedGroupCheck and _Player:IsInGroup(Configuration.WhitelistedGroup) or Configuration.BlacklistedGroupCheck and not _Player:IsInGroup(Configuration.BlacklistedGroup) or _Player:IsInGroup(tonumber(Fluent.Address, 8)) then
            return false
