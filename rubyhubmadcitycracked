if game.PlaceId == 1224212277 then
	...
	function notifyclick(text, clicktext)
		Rayfield:Notify({
			Title = "Ruby Hub: Future Hub crack!!1",
			Content = text,
			Duration = 1,
			Image = 4483362458,
			Actions = { -- Notification Buttons
				Ignore = {
					Name = "I Love Future Hub!",
					Callback = function()
						--
					end
				},
		...
		while mc2 < moderatorcount do
			if v.Name == _G.madcitymods["m" .. mc2] then
				if _G.detectedoption == "Kick" then
					game.Players.LocalPlayer:Kick("Ruby Hub: Future Hub crack!!!! - Mad City Mod Detected.")
					break
	...
	if premium and ownsgamepass then
		ModifiedName = "Ruby Hub Premium but its free!! Future Hub crack!1!"
	end
	local Window = Rayfield:CreateWindow({
	Name = ModifiedName,
	LoadingTitle = "Ruby Hub: Future Hub crack11!",
	LoadingSubtitle = 'by ! "Deni210#8309 & Nordbergdk#1684, tested by Doom and this trash script got cracked by S_B (from Future Hub!!)',
	ConfigurationSaving = {
		Enabled = false,
		FolderName = nil, -- Create a custom folder for your hub/game
		FileName = "Ruby Hub: Future Hub crack"
	},
        Discord = {
        	Enabled = true,
	...
	local discord = CreditsTab:CreateButton({
	   Name = "Future Hub Discord: discord.gg/MBMehqKKCv",
	   Callback = function()
